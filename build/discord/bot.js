...
const pterodactyl_1 = __importDefault(require("./pterodactyl"));
...
client.on('messageCreate', async (message) => {
    ...
    (0, pterodactyl_1.default)(message);
    if (command === 'help') {
        const helpMessage = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: `Update the post time`, value: `!time HH:MM` }, { name: `Channel To Post NSFW updates`, value: `!channel (Be in the channel you want updates to)` }, { name: `Amount of Posts`, value: `!posts amount` })
            .setTimestamp()
            .setFooter({
    ...
    if (command === 'activity') {
        ...
        if (!argument) {
            message.channel.send(`USAGE: !activity status`);
        }
